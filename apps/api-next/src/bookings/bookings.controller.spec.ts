...
describe("BookingsController", () => {
  const service = { list: vi.fn(), get: vi.fn(), create: vi.fn(), transition: vi.fn(), reschedule: vi.fn() };
  ...
  it.each([["confirm", BookingStatus.CONFIRMED], ["cancel", BookingStatus.CANCELLED], ["complete", BookingStatus.COMPLETED]] as const)("delegates %s transition", (method, status) => { controller[method](req, "b", { version: 2, reason: "r" }); expect(service.transition).toHaveBeenCalledWith("t", "b", "u", status, 2, "r"); });
